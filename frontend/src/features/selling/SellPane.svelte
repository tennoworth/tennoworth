<script lang="ts">
  ...
  interface Props {
    ...
    ordersSummary?: { live: number; issues: number } | null;
    marketFreshness: 'unknown' | 'fresh' | 'aging' | 'stale';
  ...
  function fmtDelta(d: number | null, unit = '') {
  ...
  function relaxFilters({ kind }: { kind: string }) {
