...
// (rust/tennoworth-desktop/src/services/sellables.rs) so the desktop tray
...
import { slugGuess, pathNameGuess, pathGuessCandidates } from './resolver';
...
describe('slugGuess / pathNameGuess parity (TS canonical side)', () => {
  ...
  it.each(fixture.guess_candidates_cases)('pathGuessCandidates($path)', (c) => {
    expect(pathGuessCandidates(c.path)).toEqual(c.expected);
  });
});
