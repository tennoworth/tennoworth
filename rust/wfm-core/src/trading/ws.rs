    ...
    match route {
        "@wfm|event/subscriptions/newOrder" => {
            ...
            let Some(platinum) = p
                .get("platinum")
                .and_then(|x| x.as_f64())
                .and_then(|price| wfm_client::unit_price(price, p.get("perTrade")))
            else {
                return WsEvent::Other;
            };
...
mod tests {
    ...
    fn streamed_lot_prices_match_the_shared_unit_price_fixture() {
        ...
        for case in cases {
            ...
            match (parse_ws_event(&frame.to_string()), case["unit"].as_f64()) {
                ...
                (WsEvent::Other, None) => (),
                (actual, expected) => {
                    panic!("price fixture mismatch: {actual:?}, expected {expected:?}")
                }
            }
        }
        let mut frame: serde_json::Value = serde_json::from_str(LIVE_ORDER).unwrap();
        frame["payload"].as_object_mut().unwrap().remove("perTrade");
        let WsEvent::NewOrder(order) = parse_ws_event(&frame.to_string()) else {
            panic!("missing lot must mean one")
        };
    ...
    fn bulk_watch_thresholds_compare_each_copy_not_the_lot_total() {
        ...
        let WsEvent::NewOrder(buy) = parse_ws_event(&frame.to_string()) else {
            panic!("expected bulk order")
        };
        ...
        let WsEvent::NewOrder(sell) = parse_ws_event(&frame.to_string()) else {
            panic!("expected bulk order")
        };
    ...
    fn parses_control_frames_and_shrugs_at_the_unknown() {
        assert_eq!(
            parse_ws_event(r#"{"route":"@wfm|cmd/subscribe/newOrders:ok"}"#),
            WsEvent::SubscribeOk
        );
        assert_eq!(
            parse_ws_event(
                r#"{"route":"@wfm|event/reports/online","payload":{"connections":36972,"authorizedUsers":20628}}"#
            ),
            WsEvent::Online { connections: 36972 }
        );
        assert_eq!(
            parse_ws_event(r#"{"route":"@wfm|event/reports/newRoute","payload":{}}"#),
            WsEvent::Other
        );
        ...
        assert_eq!(
            parse_ws_event(
                r#"{"route":"@wfm|event/subscriptions/newOrder","payload":{"type":"buy"}}"#
            ),
            WsEvent::Other
        );
    }
    ...
    fn buy_watch_fires_at_or_above_threshold_sell_at_or_below() {
        assert!(order_matches_watch(
            &order("buy", 50, Some(0), None),
            "buy",
            50,
            None,
            None
    ...
    fn tier_must_match_with_poll_path_defaults() {
        ...
        assert!(!order_matches_watch(
            &order("buy", 99, None, Some("radiant")),
            "buy",
            1,
            None,
            Some("intact")
