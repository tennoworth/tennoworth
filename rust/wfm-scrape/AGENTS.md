...
- **New items reach `path_to_info` through warframe.market's `gameRef`.**
  ...
  exact DE path from day one. `add_game_ref_paths` fills only the paths
  warframestat lacks - a lookup, not a guess - and `sets_from_recipes` derives
  a missing prime set from `ExportRecipes`, published only when its parts'
  ducats sum to the set's. A snapshot built from the prior-catalog fallback has
  no gameRefs, so both surfaces are then partial and reconcile keeps the prior
  entries.
...
`wfstat-catalog.json` and is the complete snapshot/catalog generator. With
`--observations-dir`, `scrape` also streams a per-item observation log (one
