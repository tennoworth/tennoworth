...
pub fn evaluate(w: &Watch, top: Option<&LiveTop>, now: i64) -> WatchOutcome {
    let price = top
        ...
        .and_then(|t| match w.side.as_str() {
            "sell" => t.low_sell,
            "buy" => t.top_buy,
            _ => None,
        })
...
pub fn describe(o: &WatchOutcome) -> String {
    match (o.side.as_str(), o.price) {
        ("sell", Some(p)) => format!(
            "{} is listed at {}p (you wanted ≤ {}p)",
            o.name, p, o.threshold
        ),
        ("buy", Some(p)) => format!(
            "Someone bids {}p for {} (you wanted ≥ {}p)",
            p, o.name, o.threshold
        ),
...
pub fn run_pass(app: &AppHandle) -> Vec<WatchOutcome> {
    ...
    if watches.len() > MAX_WATCHES {
        eprintln!(
            "tennoworth: watch pass: {} watches, checking the first {MAX_WATCHES}",
            watches.len()
        );
...
pub fn notify_watch(app: &AppHandle, outcome: &WatchOutcome, now: i64) -> bool {
    let mut candidate = crate::services::notifications::Candidate::once(
        format!("watch:{}", outcome.id),
        "watches",
        "TennoWorth price watch".into(),
        describe(outcome),
        "watches",
        now,
    );
    candidate.cooldown = REARM_AFTER_SECS;
    match crate::services::notifications::publish(app, candidate) {
        Ok(fired) => fired,
        Err(e) => {
...
mod tests {
    ...
    fn top(low_sell: Option<u32>, top_buy: Option<u32>) -> LiveTop {
        ...
        LiveTop {
            ...
            rank: Some(0),
            subtype: None,
            sells: low_sell.into_iter().collect(),
            buys: top_buy.into_iter().collect(),
            low_sell,
            top_buy,
            own_ask: None,
            own_bid: None,
    ...
    fn a_recently_fired_watch_stays_satisfied_but_does_not_fire_again_until_rearmed() {
        let now = NOON;
        let recent = evaluate(
            &watch("sell", 15, Some(NOON - 2 * 3600)),
            Some(&top(Some(12), None)),
            now,
        );
        assert!(recent.satisfied && !recent.fire);
        let old = evaluate(
            &watch("sell", 15, Some(NOON - REARM_AFTER_SECS - 60)),
            Some(&top(Some(12), None)),
            now,
        );
    ...
    fn describe_reads_like_a_notification() {
        ...
        assert_eq!(
            describe(&o),
            "Primed Flow is listed at 12p (you wanted ≤ 15p)"
        );
        let o = WatchOutcome {
            side: "buy".into(),
            threshold: 20,
        ...
        assert_eq!(
            describe(&o),
            "Someone bids 22p for Primed Flow (you wanted ≥ 20p)"
        );
    }
}
