    ...
    trade
        ...
        .filter_map(|item| {
            let reason = match names.get(&item.name.to_lowercase()) {
                ...
                Some(slug) if orders.iter().filter(|o| &o.slug == slug).count() > 1 => {
                    Some("multiple matching sell orders")
                }
                _ => None,
            }?;
...
pub fn handle_trade(
    ...
    if trade.kind == "sale" && !auto_close_on {
        follow_up.push("Automatic listing adjustment is off. Review your sell orders.".into());
    }
    if trade.kind == "sale" && auto_close_on {
                        ...
                        match res {
                            ...
                            Err(e) => {
                                follow_up.push(format!(
                                    "{}: listing update failed; review My Orders.",
                                    order.slug
                                ));
    ...
    if !follow_up.is_empty() {
        body.push_str(&format!(" · {}", follow_up.join(" ")));
    }
...
mod tests {
    ...
    fn follow_up_identifies_unresolved_tiers_and_ambiguous_orders() {
        ...
        let names = BTreeMap::from([
            ("primed flow".into(), "primed_flow".into()),
            ("part".into(), "part".into()),
        ]);
        let orders = vec![
            OwnSellOrder {
        ...
        let messages = adjustment_follow_up(
            &trade,
            &names,
            &orders,
            &BTreeSet::from(["primed_flow".into()]),
        );
