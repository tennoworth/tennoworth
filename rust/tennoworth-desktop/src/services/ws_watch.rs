...
fn run(app: AppHandle) {
    ...
    loop {
        ...
        let mut on_order = move |o: NewOrder| {
            ...
            outcome.fire = crate::services::watch::notify_watch(&app2, &outcome, now);
            if let Err(e) =
                db.record_watch_check(w.id, outcome.price, now, outcome.fire.then_some(now))
            {
...
mod tests {
    ...
    fn streaming_and_polling_agree_on_fractional_unit_quotes() {
        for side in ["buy", "sell"] {
            ...
            let wfm_core::trading::ws::WsEvent::NewOrder(order) =
                wfm_core::trading::ws::parse_ws_event(&frame.to_string())
            else {
                panic!("expected bulk order")
            };
            let top = wfm_core::trading::live_top::LiveTop {
                slug: "item".into(),
                rank: Some(0),
                subtype: None,
                sells: vec![7.2],
            ...
            assert_eq!(
                match_order(&order, "item", &[w], None, 1000).is_some(),
                polled.fire
            );
    ...
    fn own_orders_and_unarmed_watches_never_fire() {
        ...
        assert_eq!(
            match_order(
                &order("buy", 99, Some("Stranger")),
                "primed_flow",
                &ws,
                None,
                900 + REARM_AFTER_SECS - 1
            ),
            None
        );
        // re-armed
