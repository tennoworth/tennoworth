...
| `develop-integration.json` | Integration branch (develop) | Proposed PR-only integration with required completion gates (`audit-gate`, `ui-gate`). |
...
updates. The direct GitHub snapshot writer has been retired; release-time data
refreshes now arrive through an ordinary reviewed PR, so this snapshot can be
applied without a bot bypass. Apply the JSON with the rulesets API, then refresh
the snapshot from the live rule.
