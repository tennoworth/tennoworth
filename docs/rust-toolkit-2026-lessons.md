...
| Deny-dead-code policy | `rust/Cargo.toml` `[workspace.lints.rust]`; the platform boundary is declared at each definition rather than left as a warning on the other platform |
