...
# cannot. The one supported mode, `external-backup`, is the host-level Proxmox
# backup of this whole container, verified by an operator-run pull-and-verify job
# on another machine; nothing here is read as proof, and the report says so in as
# many words. A mode that is unset or unrecognised is not ready - an undeclared
...
case "$PRESERVATION" in
  ...
  *) error "unrecognised preservation mode '$PRESERVATION' - expected external-backup";;
esac
...
log_rows="$(mktemp)"
# One row per partial log: start|age|name. Whether a partial is a sweep still
  ...
  for path in "$OBSERVATIONS"/sweep-*; do
    ...
    case "$base" in
      *.partial)
        ...
        partial_start="$(iso_to_epoch "$(name_stamp "$base")")" || true
        printf '%s|%s|%s\n' "${partial_start:-}" "$age" "$base" >> "$partial_rows"
        continue
        ;;
    esac
...
# A sweep that dies keeps its partial log on purpose, for post-mortem, and only
# retention removes it. Once a later sweep has completed, that partial belongs to
# a sweep that is over: the evidence it lost is already counted as a missing
# sweep below, so it is reported, not failed a second time. Only a partial that
# nothing has superseded can be a sweep still stuck, and one whose start cannot
# be read is judged by its age alone rather than excused.
abandoned=()
stuck_age=""
newest_start_epoch=""
[ -n "$newest_start" ] && newest_start_epoch="$(iso_to_epoch "$newest_start")"
while IFS='|' read -r partial_start age name; do
  [ -n "$name" ] || continue
...
if [ -n "$stuck_age" ]; then
  error "a partial observation log is $((stuck_age / 60)) minutes old - a sweep is stuck"
fi
if [ "${#abandoned[@]}" -gt 0 ]; then
  warn "${#abandoned[@]} partial log(s) from sweeps that did not complete are kept for post-mortem: ${abandoned[*]}"
fi
...
# verified by an operator-run pull-and-verify job on another machine; nothing
# here can see it, so the report says declared-external and records that it is
# not independently verified from this host - it must never read as
...
report="$(cat <<EOF
{
  ...
  "anomalies": {
    ...
    "abandoned_partial": $(jarr "${abandoned[@]+"${abandoned[@]}"}")
  },
  "completeness": {
