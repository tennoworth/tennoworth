...
for f in run-scrape.sh alert.sh pull-app.sh pull-web.sh pull-scrape.sh pull-policy.sh; do
  src="deploy/$f"
  [ -f "$src" ] || continue
...
for u in wfm-scrape wfm-app-pull wfm-web-pull wfm-scrape-pull wfm-policy-pull; do
