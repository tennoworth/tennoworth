# scripts/ — three Node tools + the shared fixtures
...
remains in this directory is three Node tools; the shared cross-language
...
- `check-panic-sites.mjs` — the unwrap/expect gate (audit.yml). Fails on any
...
- `check-probe-report.mjs` — the gate for the TENNOWORTH_PROBE UI smoke run
