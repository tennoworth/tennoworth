...
const MAX_ASSISTANT_QUESTION_CHARS: usize = 2000;
const MAX_ASSISTANT_CONTEXT_CHARS: usize = 100_000;
const MAX_ASSISTANT_HISTORY_ENTRIES: usize = 12;
// MAX_BODY_BYTES (64 KB, below) would truncate a legitimate max-size context
// before it ever reaches the too_large check — this route needs its own cap.
const MAX_ASSISTANT_BODY_BYTES: u64 = 512 * 1024;
...
const MAX_ASSISTANT_CALLS: usize = 20;
const ASSISTANT_RATE_WINDOW: Duration = Duration::from_secs(60);
...
struct AssistantMessage {
    role: String, // client-supplied; sanitized to "user"/"assistant" before use
    content: String,
}
...
fn cap_history(mut history: Vec<AssistantMessage>) -> Vec<AssistantMessage> {
...
fn build_assistant_messages(context: &str, history: &[AssistantMessage], question: &str) -> Vec<serde_json::Value> {
...
fn assistant_rate_limited(calls: &mut VecDeque<Instant>, now: Instant) -> bool {
...
fn resolve_deepseek_key(env_value: Option<&str>, config_dir: &Path) -> Option<String> {
...
fn deepseek_client() -> Result<Client> {
    Client::builder()
        .user_agent(BROWSER_UA)
...
fn deepseek_request_body(messages: &[serde_json::Value]) -> serde_json::Value {
    serde_json::json!({
        "model": "deepseek-chat",
        "temperature": 0.3,
        "max_tokens": 1024,
...
fn call_deepseek(client: &Client, api_key: &str, messages: Vec<serde_json::Value>) -> Result<(String, AssistantUsage)> {
    let resp = client
...
mod tests {
    ...
    fn tmp_dir(name: &str) -> PathBuf {
        let mut p = env::temp_dir();
        p.push(format!("wfminv-test-dir-{}-{}", std::process::id(), name));
    ...
    // ---- /assistant: system-prompt / message assembly ----
    ...
    // ---- /assistant: prompt-injection defense (role sanitization) ----
    ...
    // ---- /assistant: error detail truncation ----
