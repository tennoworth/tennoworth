...
fn find_root_from(mut dir: PathBuf) -> Result<PathBuf, String> {
    loop {
        ...
        if dir.join(".git").exists() && dir.join("prototype").join("public").is_dir() {
            return Ok(dir);
        }
        match dir.parent() {
...
mod tests {
    ...
    fn locates_linked_worktrees_without_requiring_generated_csv() {
        ...
        std::fs::create_dir_all(root.join("prototype/public")).unwrap();
        std::fs::create_dir_all(root.join("companion/wfm-scrape")).unwrap();
        std::fs::write(root.join(".git"), "gitdir: unused-for-discovery").unwrap();
        let found = find_root_from(root.join("companion/wfm-scrape"));
