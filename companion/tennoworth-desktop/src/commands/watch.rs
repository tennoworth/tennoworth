...
pub fn list_watches(db: State<'_, Db>) -> Result<Vec<Watch>, CmdError> {
    db.list_watches()
        .map_err(|e| CmdError::internal(format!("list watches: {e}")))
}
...
pub fn add_watch(db: State<'_, Db>, watch: NewWatch) -> Result<Vec<Watch>, CmdError> {
    ...
    if existing.len() >= MAX_WATCHES {
        return Err(CmdError::of(
            "too_many",
            format!("at most {MAX_WATCHES} watches - delete one first"),
        ));
    }
    db.add_watch(&watch, None)
        .map_err(|e| CmdError::internal(format!("add watch: {e}")))?;
    db.list_watches()
        .map_err(|e| CmdError::internal(format!("list watches: {e}")))
}
...
pub fn delete_watch(db: State<'_, Db>, id: i64) -> Result<Vec<Watch>, CmdError> {
    db.delete_watch(id)
        .map_err(|e| CmdError::internal(format!("delete watch: {e}")))?;
    db.list_watches()
        .map_err(|e| CmdError::internal(format!("list watches: {e}")))
}
