...
pub enum Confidence { Scanned, Tracked, Estimated, Unknown }
...
impl Observation {
    pub fn scanned(
        account_key: String, snapshot_id: i64,
        raw: &serde_json::Value, before: Option<LogPosition>, after: Option<LogPosition>,
        started_at: i64, observed_at: i64,
    ) -> Self {
        let integer = |key: &str| raw.get(key).and_then(|v| v.as_u64())
        ...
        let mut result = Self {
            account_key, run_id: String::new(), remaining: remaining.or(mastery_rank), mastery_rank,
            observed_at, utc_day: observed_at.div_euclid(86_400), snapshot_id,
            confidence: if remaining.is_some() { Confidence::Scanned } else if mastery_rank.is_some() {
                Confidence::Estimated
            } else { Confidence::Unknown },
            reason: remaining.is_none().then(|| "Scan did not contain a usable remaining-trade count.".into()),
        ...
        if started_at.div_euclid(86_400) != result.utc_day {
            ...
            result.reason = Some("Remaining count is from the scan; log monitoring is unavailable.".into());
        } else if !monitoring && mastery_rank.is_some() {
            result.reason = Some("Remaining count was missing; using mastery as an estimate without log monitoring.".into());
        }
        result
    }

    pub fn mark_uncertain(&mut self, reason: &str) {
        self.confidence = if self.remaining.is_some() { Confidence::Estimated } else { Confidence::Unknown };
    ...
    pub fn rollover(&mut self, now: i64) {
        ...
        if day > self.utc_day {
            ...
            self.confidence = if self.remaining.is_some() { Confidence::Estimated } else { Confidence::Unknown };
            self.reason = Some("UTC reset: mastery-based estimate; scan to confirm the account allowance.".into());
    ...
    pub fn reconcile(&mut self, position: &LogPosition, now: i64) {
        self.rollover(now);
        let (Some(before), Some(after)) = (&self.before, &self.after) else { return };
        if position.session != after.session {
            self.mark_uncertain("Game log changed; scan again to confirm the account and allowance.");
            return;
        }
        if position.end <= before.end { return; }
        ...
        if self.confidence == Confidence::Scanned { self.confidence = Confidence::Tracked; }
    }
    ...
    pub fn view(&self, run_id: &str, now: i64) -> AllowanceView {
        ...
        if current.run_id != run_id {
            current.mark_uncertain("Monitoring restarted; scan again to confirm the account and remaining trades.");
        }
        AllowanceView {
            remaining: current.remaining, mastery_rank: current.mastery_rank,
            observed_at: Some(current.observed_at), utc_day: current.utc_day,
            snapshot_id: Some(current.snapshot_id), confidence: current.confidence,
            reason: current.reason, monitoring: current.monitoring,
        }
    }
}

pub fn unknown(now: i64) -> AllowanceView {
    AllowanceView { remaining: None, mastery_rank: None, observed_at: None,
        utc_day: now.div_euclid(86_400), snapshot_id: None, confidence: Confidence::Unknown,
        reason: Some("Scan the game to read its remaining trade allowance.".into()), monitoring: false }
}
...
mod tests {
    ...
    fn event_name_matches_the_frontend_fixture() {
        let fixture: serde_json::Value = serde_json::from_str(include_str!("../../../tests/fixtures/trade-session/events.json")).unwrap();
        assert_eq!(fixture["allowance_changed"], EVENT_ALLOWANCE_CHANGED);
    }

    fn position(start: u64, end: u64, observed_after: i64) -> LogPosition {
        LogPosition { session: "session".into(), start, end, observed_after }
    }

    fn scan(raw: serde_json::Value) -> Observation {
        let mut observation = Observation::scanned("account".into(), 1, &raw,
            Some(position(100, 100, 100)), Some(position(200, 200, 102)), 100, 102);
    ...
    fn missing_and_invalid_metadata_do_not_turn_into_zero() {
        for raw in [serde_json::json!({}), serde_json::json!({"TradesRemaining":-1}),
            serde_json::json!({"TradesRemaining":"4"}), serde_json::json!({"TradesRemaining":4.5})] {
    ...
    fn scan_crossing_midnight_does_not_claim_authority_for_either_day() {
        let observed = Observation::scanned("account".into(), 1,
            &serde_json::json!({"TradesRemaining":3,"PlayerLevel":24}), None, None, 86_399, 86_401);
        assert_eq!(observed.remaining, Some(24));
        assert_eq!(observed.confidence, Confidence::Estimated);
    }
}
